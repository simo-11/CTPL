...
void first(int id) {
	std::cout << "hello from first-function: id=" << id << std::endl;
}

void aga(int id, int par) {
	std::cout << "hello from aga-function" << id << 
		" with parameter " << par << std::endl;
}

struct Third {
	Third(int v) { 
		this->v = v; 
		std::cout << "Third ctor " << this->v << std::endl; 
	}
	Third(Third&& c) { 
		this->v = c.v; 
...
void mmm(int id, const std::string& s) {
	std::cout << "mmm function " << id << ' ' << s << std::endl;
}

void ugu(int id, Third& t) {
	std::this_thread::sleep_for(std::chrono::milliseconds(2000));
	std::cout << "hello from " << id << 
		", function with parameter Third " << t.v << std::endl;
}
