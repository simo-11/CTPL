...
namespace ctpl {
    ...
    public:
        ...
        /** change the number of threads in the pool
         should be called from one thread, otherwise be 
         careful to not interleave, also with this->stop()
         nThreads must be >= 0
        */
        void resize(int nThreads) {
            if (!this->isStop && !this->isDone) {
                ...
                if (oldNThreads <= nThreads) {  
                    ...
                    this->flags.resize(nThreads);  
                    /** safe to delete because the threads have copies 
        ...
        waiting.
		if isWait == true, all the functions in the queue are run, otherwise
		the queue is cleared without running the functions
		*/
		void stop(bool isWait = false) {
            ...
            // if there were no threads in the pool but some functors 
            // in the queue, the functors are not deleted by the threads
        ...
        auto push(F && f, Rest&&... rest) ->std::future<decltype
            (f(0, rest...))> {
        ...
        // run the user's function that excepts argument int - 
        // id of the running thread. returned value is templatized
        // operator returns std::future, where the user can get the 
        // result and rethrow the catched exceptins
        template<typename F>
        auto push(F && f) ->std::future<decltype(f(0))> {
            auto pck = std::make_shared<std::packaged_task
                <decltype(f(0))(int)>>(std::forward<F>(f));
            auto _f = new std::function<void(int id)>([pck](int id) {
                (*pck)(id);
            });
            this->q.push(_f);
    ...
    private:
        ...
        void set_thread(int i) {
            ...
            auto f = [this, i, 
                flag/* a copy of the shared ptr to the flag */]() {
                ...
                while (true) {
                    while (isPop) {  // if there is anything in the queue
                        std::unique_ptr<std::function<void(int id)>> func(_f); 
                        // at return, delete the function even 
                        // if an exception occurred
                        (*_f)(i);
                        if (_flag)
                            // the thread is wanted to stop, 
                            // return even if the queue is not empty yet
                            return;
                        else
                            isPop = this->q.pop(_f);
                    }
